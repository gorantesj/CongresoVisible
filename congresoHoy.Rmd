...
Los siguientes datos son obtenidos de manera provisional a través de un filtro del estado del proyectos de ley, pero se debe generar la variable correspondiente.

Audiencias públicas citadas: `r tbl(con, "proyecto_leys") %>%
  # filter(activo == 1 && cuatrienio_id == 1) %>%
  count(estado_actual_id) %>%
  left_join(tbl(con, "estado_proyecto_leys") %>%
              select(estado_actual_id = id, estado = nombre) ) %>%
  filter(estado %in% c("Audiencia Pública" ,"Solicitud Audiencia Pública" )) %>%
  select(n) %>% summarise(n = sum(n)) %>%  collect()`
  
Debates de Control Político citados: Este dato no se puede obtener

Sentencias emitidas: `r tbl(con, "proyecto_leys") %>%
  # filter(activo == 1 && cuatrienio_id == 1) %>%
  count(estado_actual_id) %>%
  left_join(tbl(con, "estado_proyecto_leys") %>%
              select(estado_actual_id = id, estado = nombre) ) %>%
...
Objeciones presentadas: `r tbl(con, "proyecto_leys") %>%
  # filter(activo == 1 && cuatrienio_id == 1) %>%
  count(estado_actual_id) %>%
  left_join(tbl(con, "estado_proyecto_leys") %>%
              select(estado_actual_id = id, estado = nombre) ) %>%
...
Proyectos de Ley radicados: `r tbl(con, "proyecto_leys") %>%
...
## Con mayor número de autorías de Proyectos de Ley
```{r warning=FALSE, echo=FALSE, comment=FALSE, message=FALSE}
tbl(con, "proyecto_ley_autors") %>%
  ...
  count(corporacion, autor_id) %>%  arrange(desc(n)) %>% head(10) %>%  collect() %>% 
  mutate(autor = as.character(autor_id)) %>% 
  hchart(hcaes(x = autor, y = n, group = corporacion), type = "bar")
```

## Con mayor número de citaciones de Debates de Control Político
No encontré nada que una citaciones con congresistas 

## Total de autorías
No se puede porque no hay info entre congrsista_id y autor_id 

## Temas recurrentes
```{r warning=FALSE, echo=FALSE, comment=FALSE, message=FALSE}

tbl(con, "proyecto_leys") %>%  
  count(tema_principal_id, camara_id) %>% 
  left_join( tbl(con, "temas") %>%  select(tema_principal_id = id, tema = nombre)) %>% 
  arrange(desc(n)) %>%  head(20) %>%  collect() %>% arrange(camara_id) %>% 
  hchart(hcaes(x = tema, value = n, color = camara_id), type = "treemap")
```
