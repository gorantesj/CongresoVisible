...
```{r}
 ...
 OVER ()) THEN (`estado_proyecto`) WHEN NOT(`n` > AVG(`n`) OVER ()) THEN ('') END
  show_query()
...
```{r}
aver <- tbl(con, "proyecto_ley_autor_legislativos") %>% 
  select(proyecto_ley_id, congresista_id) %>% 
left_join(tbl(con, "proyecto_leys") %>% 
 ...
 left_join(
 tbl(con, "partidos") %>%  select(id, color, partido=nombre),
 by = c("partido_id" = "id")
  ) %>%  
  left_join(
  tbl(con, "temas") %>%  select(id, tema = nombre),
...
tbl(con, "proyecto_leys") %>% 
  select(id, legislatura_id, corporacion_id, tipo_proyecto_id, tema_id_principal) %>% 
  left_join(tbl(con, "temas") %>%  select(id, tema = nombre),
  by = c("tema_id_principal" = "id")
  ) %>% 
  count(tema) %>% 
  arrange(desc(n)) %>% 
...
aver  %>% 
  hchart(hcaes(from = tema, to  = partido, weight =n), type = "sankey") %>%
